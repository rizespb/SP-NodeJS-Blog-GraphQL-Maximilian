...
class Feed extends Component {
  ...
  componentDidMount() {
    ...
    fetch('http://localhost:8080/graphql', {
      ...
      .then((resData) => {
        if (resData.errors) {
          throw new Error('Fetching status failed at componentDidMount Feed.js')
        }
  ...
  finishEditHandler = (postData) => {
    ...
    fetch('http://localhost:8080/post-image', {
      ...
      .then((resData) => {
        if (resData.erorrs && resData.erorrs[0].status === 422) {
          throw new Error('Validation failed in finishEditHandler')
        }

        if (resData.errors) {
          throw new Error('Post creation failed at finishEditHandler')
        }
  ...
  deletePostHandler = (postId) => {
    ...
    fetch('http://localhost:8080/graphql', {
      ...
      .then((resData) => {
        if (resData.errors) {
          throw new Error('Deleting the post failed at deletePostHandler')
        }
